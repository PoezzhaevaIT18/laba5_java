...
Вставляем элементы списка L2 в L1 не нарушая его упорядоченности, то есть в конце должен быть итоговый упорядоченный список состоящий из всех элементов. Используется 2 типа переменных: Integer и String, используем обобщеный метод. Оба описаны в примере работы программы.
Пример:
Работа с числами: 
